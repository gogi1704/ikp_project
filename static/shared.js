...
export function linkActions(link){
  return link.url?`<a class="text-button" href="${esc(link.url)}" target="_blank" rel="noopener noreferrer">Открыть</a><button type="button" class="text-button" data-copy-link="${esc(link.url)}">Скопировать</button>`:'<small class="muted">Ссылка создана до обновления</small>';
}
export function wireLinkCopyButtons(root=document){
  root.querySelectorAll('[data-copy-link]').forEach(button=>button.onclick=async()=>{try{await navigator.clipboard.writeText(button.dataset.copyLink);notify('Ссылка скопирована');}catch{notify('Не удалось скопировать ссылку');}});
}
