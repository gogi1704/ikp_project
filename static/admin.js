import {$,esc,date,rub,api,setCsrf,notify,field,messengerFields,wireMessengerFields,readMessengerFields,submissionDetail} from './shared.js';
const app=$('#app');
...
function renderActivity(){
  ...
  $('#admin-activity').innerHTML=filtered.map(group=>{
    const links=group.links.map(link=>`<div class="activity activity-row"><div><span class="pill ${link.revoked?'':'green'}">${link.revoked?'Отозвана':link.viewed?'Просмотрена':'Опубликована'}</span><small>Создана ${date(link.created)}${link.viewed?' · просмотрена '+date(link.viewed):' · ещё не просмотрена'}</small></div></div>`).join('');
