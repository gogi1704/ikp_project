...
function render(){
  ...
  $('#proposal-form').oninput=e=>{
    ...
    if(e.target.name==='count'&&Number(e.target.value)>=1){
      const count=Number(e.target.value);
      document.querySelectorAll('.addon-qty').forEach(input=>{input.max=String(count);if(Number(input.value)>count)input.value=String(count);});
    }
