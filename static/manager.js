...
function render(){
  ...
  allActivity();
  if(!p)return;
  $('#proposal-form').oninput=e=>{if(e.target.id==='photo')return;readForm();dirty=true;$('#save-status').textContent='Есть изменения';updateSum();};
  setupInnSuggestions();
...
function readForm(){const p=current.body;for(const input of $('#proposal-form').querySelectorAll('input[name]')){const parts=input.name.split('.');const val=input.type==='checkbox'?input.checked:input.type==='number'?Number(input.value):input.value;if(parts.length===3)p[parts[0]][parts[1]][parts[2]]=val;else p[input.name]=val;}}
...
async function save(){
  ...
  }finally{saving=false;inputs.forEach(input=>input.disabled=false);}
}
