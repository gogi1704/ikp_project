...
def proposal(data, publish=False):
    ...
    for code, _, price in ADDONS:
        ...
        qty = integer(item.get('qty', 0))
        if qty > p['count']:
            raise ValueError('Количество услуг не может превышать число сотрудников')
        p['addons'][code] = {'on': boolean(item.get('on', True)), 'qty': qty, 'price': whole_rubles(item.get('price', price))}
...
def selection(p, data=None):
    ...
    for group, catalog in [('corp', CORP), ('health', HEALTH), ('addons', ADDONS)]:
        ...
        for code, _, price in catalog:
            ...
            item = data.get(group, {}).get(code, {})
            preset = p.get('addons', {}).get(code, {'on': True, 'qty': 0}) if group == 'addons' else {}
            on = boolean(item.get('on', preset.get('on', True) if group == 'addons' else offered))
            ...
            qty = integer(item.get('qty', preset.get('qty', 0) if group == 'addons' else 2 if code == 'liverKidney' else s['count']))
